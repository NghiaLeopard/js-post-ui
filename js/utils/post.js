...
export function createLiElement(data) {
  ...
  setTextContent(liElement, '[data-id="description"]', truncateDesc(data.description, 100))
  setTextContent(liElement, '[data-id="author"]', data.author)
  setTextContent(liElement, '[data-id="timeSpan"]', `- ${dayjs(data.updatedAt).fromNow()}`)
  ...
  divElement.addEventListener('click', (event) => {
    if (event.target.dataset.id === 'edit' || event.target.classList.contains('fa-pen')) return
